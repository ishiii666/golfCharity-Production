...
export default function Footer() {
    ...
    const [dynamicContent, setDynamicContent] = useState({
        copyright: `© ${currentYear} GOLFCHARITY. All rights reserved.`,
        tagline: 'Where precision meets purpose. Play golf, win prizes, and support incredible charities across Australia.'
    });
    ...
    useEffect(() => {
        async function fetchContent() {
            try {
                ...
                if (content.length > 0) {
                    const footerItems = content.filter(c => c.section_id === 'footer');
                    const updates = {};
                    footerItems.forEach(item => {
                        if (item.field_name === 'copyright') updates.copyright = item.field_value;
                        if (item.field_name === 'tagline') updates.tagline = item.field_value;
                    });
                    setDynamicContent(prev => ({ ...prev, ...updates }));
                }
            } catch (err) {
